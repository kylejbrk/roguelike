...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionHandler]:
        ...
        if event.sym in { # Ignor modifier keys.
            tcod.event.K_LSHIFT,
            tcod.event.K_RSHIFT,
            tcod.event.K_LCTRL,
            tcod.event.K_RCTRL,
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key in MOVE_KEYS:
            ...
            if event.mod & (tcod.event.KMOD_SHIFT | tcod.event.KMOD_RSHIFT):
                modifier *= 5
...
class AreaRangedAttackHandler(SelectIndexHandler):
    '''Handles targeting an area within a given radius. Any entity within the are will be affected.'''
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
        ...
        if event.sym in CURSOR_Y_KEYS:
            ...
            if adjust < 0 and self.cursor == 0:
                # Only move from the tope to the bottom when you're on the edge.
                self.cursor = self.log_length -1
